    ...
    <>
      ...
      <div className="bg-[#F9F9F9] py-10 px-6 sm:px-20">
        <nav className="text-sm text-gray-500">
          ...
          <span className="text-gray-400">Energy Boosters</span>
        </nav>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {filteredProducts.map((product) => (
              <div key={product.id} className="group relative">
                ...
                <Link href={`/multivitamins/${product.id}`}>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-3xl relative flex flex-col md:flex-row shadow-2xl">
            ...
            <div className="md:w-1/2 p-10 flex flex-col justify-center">
              ...
              <Link href={`/multivitamins/${quickViewProduct.id}`} className="text-sm font-semibold underline flex items-center gap-1 hover:text-[#51ADE5]">
                View full details ↗
              </Link>
            </div>
          </div>
